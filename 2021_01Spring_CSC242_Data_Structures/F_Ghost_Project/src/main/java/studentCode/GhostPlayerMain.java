...
public class GhostPlayerMain {
	...
	public static final String DICTIONARY_NAME = "ARBITER_DICTIONARY.txt";
	public static final String DICTIONARY_FILE_PATH = File.separator+DICTIONARY_NAME;
	...
	public void update() throws IOException, IllegalStateException{
		...
		nextUpdateTime = System.currentTimeMillis() + 250;
	...
	public static class Dictionary {
		...
		public Dictionary(String fileDirectory) throws IOException {
			...
			try {
				bufferedReader = new BufferedReader(new InputStreamReader(GhostPlayerMain.class.getResourceAsStream(fileDirectory)));
			}catch (NullPointerException e) {
				try {
					System.out.println("Team "+myTeamName+" Unable to load dictionary internally. Downloading file...");
					bufferedReader = new BufferedReader(new InputStreamReader(new BufferedInputStream(new URL("https://github.com/dwyl/english-words/blob/master/words.txt?raw=true").openStream()), StandardCharsets.UTF_8));
				}catch (Exception e1){
					throw new FileNotFoundException("Dictionary Loading error. Team "+myTeamName+", unable to load the dictionary.");
				}
			}
